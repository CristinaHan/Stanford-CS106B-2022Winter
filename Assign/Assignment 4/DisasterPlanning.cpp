...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if (numCities < 0) {
        error("The number of cities must be non-negative!");
    }
    ...
    for (auto city: roadNetwork) {
        ...
        for (string supplied: supplyLocations) {
            if (supplied == city || roadNetwork[supplied].contains(city)) {
                notCovered = false;
                break;
            }
        }
        if (notCovered) {
            if (numCities >= 1) {
                auto suppliedSelf = supplyLocations + city;
                if (canBeMadeDisasterReady(roadNetwork, numCities - 1, suppliedSelf)) {
                    supplyLocations = suppliedSelf;
                    return true;
                }
                else {
                    for (string adjacentCity: roadNetwork[city]) {
                        auto suppliedAdja = supplyLocations + adjacentCity;
                        if (canBeMadeDisasterReady(roadNetwork, numCities - 1, suppliedAdja)) {
                            supplyLocations = suppliedAdja;
                            return true;
                        }
                    }
                    return false;
