...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    if (possibleLinks[personOne].isEmpty()) {
        return false;
    }
    for (auto personTwo: possibleLinks[personOne]) {
        Pair newPair(personOne, personTwo);
        Map<string, Set<string>> newLinks = possibleLinks;
        newLinks.remove(personOne);
        newLinks.remove(personTwo);
        for (auto key: newLinks) {
            if (newLinks[key].contains(personOne)) {
                newLinks[key] -= personOne;
            }
            if (newLinks[key].contains(personTwo)) {
                newLinks[key] -= personTwo;
            }
        }
        if (hasPerfectMatching(newLinks, matching)) {
            matching += newPair;
            return true;
        }
    }
...
int weightsOfAll(const Map<string, Map<string, int>>& possibleLinks,
                 const Set<Pair>& maxPairs) {
    int weights = 0;
    for (auto matched: maxPairs) {
        weights += possibleLinks[matched.first()][matched.second()];
    }
    return weights;
}
...
Set<Pair> maximumWeightMatchingRec(const Map<string, Map<string, int>>& possibleLinks,
                                   Set<Pair>& matching) {
    Map<Set<Pair>, int> weightsOfPairs;
    /* Base Case */
    if (possibleLinks.isEmpty()) {
        return matching;
    }
    ...
    string personOne = possibleLinks.firstKey();
    if (possibleLinks[personOne].isEmpty()) {
        auto newLinks = possibleLinks;
        newLinks.remove(personOne);
        return maximumWeightMatchingRec(newLinks, matching);
    }
    for (auto personTwo: possibleLinks[personOne]) {
        if (possibleLinks[personOne][personTwo] >= 0) {
            Pair newPair(personOne, personTwo);
            auto newMatching = matching + newPair;
            auto newLinks = possibleLinks;
            newLinks.remove(personOne);
            newLinks.remove(personTwo);
            for (auto key: newLinks) {
                if (newLinks[key].containsKey(personOne)) {
                    newLinks[key].remove(personOne);
                }
                if (newLinks[key].containsKey(personTwo)) {
                    newLinks[key].remove(personTwo);
                }
            }
            auto matchedPairs = maximumWeightMatchingRec(newLinks, newMatching) + matching;
            weightsOfPairs[matchedPairs] = weightsOfAll(possibleLinks, matchedPairs);
        }
    }

    auto nextLinks = possibleLinks;
    nextLinks.remove(personOne);
    for (auto key: nextLinks) {
        if (nextLinks[key].containsKey(personOne)) {
            nextLinks[key].remove(personOne);
        }
    }

    auto nextMaxPairs = maximumWeightMatchingRec(nextLinks, matching);
    weightsOfPairs[nextMaxPairs] = weightsOfAll(possibleLinks, nextMaxPairs);
    matching = weightsOfPairs.firstKey();
    for (auto setOfPairs: weightsOfPairs) {
        if (weightsOfPairs[setOfPairs] >= weightsOfPairs[matching]) {
            matching = setOfPairs;
        }
    }
    return matching;
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    /* TODO: Delete this comment and these remaining lines, then implement this function. */
    Set<Pair> matchingInit= {};
    return maximumWeightMatchingRec(possibleLinks, matchingInit);
}
