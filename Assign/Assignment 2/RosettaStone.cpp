...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
     ...
     * this function.
     */
    Map<string, double> kGramstrFreq;
    if (kGramLength <= 0) {
        error("The k-gram length must be positive!");
    } else {
        if (kGramLength > str.length()) {
            return kGramstrFreq;
        }
        int i = 0;
        while (i <= str.length() - kGramLength) {
            kGramstrFreq[str.substr(i, kGramLength)] += 1;
            i++;
        }
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
     ...
     * this function.
     */
    Map<string, double> freqNormalize;
    double total = 0;
    for (string key: input) {
        total += input[key] * input[key];
    }
    if (total == 0) {
        error("The Map is empty or has no nonzero values");
    } else {
        total = sqrt(total);
        for (string key: input) {
            freqNormalize[key] = input[key] / total;
        }
    }
    return freqNormalize;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
     ...
     * this function.
     */
    Map<string, double> topKGrams = source;
    PriorityQueue<string> sourceInQueue;
    if (numToKeep < 0) {
        error("The numToKeep must be non-negative!");
    } else if (numToKeep == 0) {
        (void) topKGrams.clear();
        return topKGrams;
    } else if (numToKeep >= source.size()) {
        return source;
    } else {
        for (string key: source) {
            sourceInQueue.enqueue(key, source[key]);
        }
        while (sourceInQueue.size() > numToKeep) {
            string strToPop = sourceInQueue.dequeue();
            topKGrams.remove(strToPop);
        }
    }
    return topKGrams;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()) {
        ...
        for (Corpus nameAndProfile: corpora) {
            if (cosineSimilarityOf(textProfile, nameAndProfile.profile) >= maxSimilarity) {
                maxSimilarity = cosineSimilarityOf(textProfile, nameAndProfile.profile);
                result = nameAndProfile.name;
            }
        }
    }
