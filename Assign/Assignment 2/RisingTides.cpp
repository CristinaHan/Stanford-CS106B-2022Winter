...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Queue<GridLocation> waterPath;
    if (sources.isEmpty()) {
        return flood;
    }
    for (GridLocation location: sources) {
        if ((terrain.get(location) <= height) && (terrain.inBounds(location))) {
            flood.set(location, true);
            waterPath.enqueue(location);
        }
    }
    GridLocation floodLocation;
    while (!waterPath.isEmpty()) {
        floodLocation = waterPath.dequeue();
        GridLocation moveUp = { floodLocation.row + 1, floodLocation.col };
        GridLocation moveDown = { floodLocation.row - 1, floodLocation.col };
        GridLocation moveRight = { floodLocation.row, floodLocation.col + 1 };
        GridLocation moveLeft = { floodLocation.row, floodLocation.col - 1};
        Vector<GridLocation> nextLocation = { moveUp, moveDown, moveRight, moveLeft };
        for (GridLocation move: nextLocation) {
            if (terrain.inBounds(move) && (!flood.get(move)) && (terrain.get(move) <= height)) {
                    flood.set(move, true);
                    waterPath.enqueue(move);
            }
        }
    }
    return flood;
