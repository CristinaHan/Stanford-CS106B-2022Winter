 ...
 */
Set<string> allEmphasesRec(const Vector<string>& remaining,
                           const Vector<string>& doneSoFar) {
    Set<string> result;
    if (remaining.isEmpty()) {
        string sentenceDone = "";
        for (string word: doneSoFar) {
            sentenceDone += word;
        }
        result += sentenceDone;
    }
    else {
        Vector<string> remainingCopy = remaining;
        string word = remainingCopy.remove(0);
        if (!isalpha(word[0])) {
            result += allEmphasesRec(remainingCopy, doneSoFar + word);
        }
        else {
            string wordUpper = toUpperCase(word);
            string wordLower = toLowerCase(word);
            result += allEmphasesRec(remainingCopy, doneSoFar + wordUpper);
            result += allEmphasesRec(remainingCopy, doneSoFar + wordLower);
        }
    }
    return result;
}
Set<string> allEmphasesOf(const string& sentence) {
    ...
    Vector<string> wordMade = {};
    Vector<string> remaining = tokenize(sentence);
    return allEmphasesRec(remaining, wordMade);
}
...
STUDENT_TEST("Test a sentence with number") {
    Set<string> expected = {
        "??do you have 5 rollsrace?!",
        "??do you have 5 ROLLSRACE?!",
        "??do you HAVE 5 rollsrace?!",
        "??do you HAVE 5 ROLLSRACE?!",
        "??do YOU have 5 rollsrace?!",
    ...
    EXPECT_EQUAL(allEmphasesOf("??Do you have 5 RollsRace?!"), expected);
}
