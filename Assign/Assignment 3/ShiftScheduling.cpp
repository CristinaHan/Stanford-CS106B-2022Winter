...
Set<Shift> highestValueScheduleRec(const Set<Shift>& remaining,
                                   const Set<Shift>& shiftHave,
                                   int maxHours) {
    if (remaining.isEmpty()) {
        return shiftHave;
    }
    Shift shiftToMake = remaining.first();
    Set<Shift> remainingNow = remaining - shiftToMake;
    bool addAllowed = true;
    int totalHours = 0;
    for (Shift shiftNow: shiftHave) {
        if (overlapsWith(shiftToMake, shiftNow)) {
            addAllowed = false;
            break;
        }
        totalHours += lengthOf(shiftNow);
    }
    if (totalHours + lengthOf(shiftToMake) > maxHours) {
        addAllowed = false;
    }
    if (addAllowed) {
        Set<Shift> addShift = highestValueScheduleRec(remainingNow, shiftHave + shiftToMake, maxHours);
        Set<Shift> notAddShift = highestValueScheduleRec(remainingNow, shiftHave, maxHours);
        int valueAdd = 0, valueNotAdd = 0;
        for (Shift elem: addShift) {
            valueAdd += valueOf(elem);
        }
        for (Shift elem: notAddShift) {
            valueNotAdd += valueOf(elem);
        }
        if (valueAdd >= valueNotAdd) {
            return addShift;
        }
        else {
            return notAddShift;
        ...
        return highestValueScheduleRec(remainingNow, shiftHave, maxHours);
    }
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    if (maxHours < 0) {
        error("The total hours is negative!!");
    }
    else if (maxHours == 0) {
        return {};
    }
