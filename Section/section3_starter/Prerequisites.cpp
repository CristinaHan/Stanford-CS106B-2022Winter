...
void listLegalOrderingsRec(const Map<string, Set<string>>& prereqs,
                             const Vector<string>& coursePerformed,
                             const Set<string>& remaining) {
    if (remaining.isEmpty()) {
        cout << coursePerformed << endl;
        return;
    }
    Set<string> courseDone;
    for (string course: coursePerformed) {
        courseDone += course;
    }
    for (string course: remaining) {
        if (prereqs[course].isSubsetOf(courseDone)) {
            Vector<string> newCourse = coursePerformed;
            newCourse += course;
            listLegalOrderingsRec(prereqs, newCourse, remaining - course);
        }
    }
}
